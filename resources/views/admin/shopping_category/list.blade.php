        ...
        <div class="col-sm-12 col-xl-12">
            <div class="bg-secondary rounded h-100 p-4">
                <div style="display: flex; justify-content: space-between;">
                    ...
                    <div class="InputContainer" style="margin-right: 200px;">
                        <form action="" method="get">
                            <input  placeholder="Search.." class="input" name="keyword" value="{{ is_null(request()->keyword) ? "" : request()->keyword}}">
                        </form>
                    </div>
                </div>
                <form action="{{ route('admin.shopping_category.deleteAll') }}" method="post">
                    @csrf
                    <div class="function" style="display: flex;">
                        ...
                        <div class="col-lg-8 col-md-12" >
                            <a class="btn btn-primary" href="{{ route('admin.shopping_category.create') }}">Create Shopping category</a>
                        </div>
                    </div>
                    <table class="table">
                        <thead>
                        ...
                        <tbody>
                            @forelse ($product_Categories as $productCategory)
                                <tr>
                                    <th><input name='id[]' id="selectAll" value="{{ $productCategory->id }}" type="checkbox">
                                    </th>
                                    ...
                                    <td>
                                        <form method="post" action="{{route('admin.shopping_category.delete', ['id' => $productCategory->id])}}">
                                            @csrf
                                            {{ method_field('delete') }}
                                            <a href=" {{route('admin.shopping_category.detail', ['id' => $productCategory->id])}}" class="btn btn-primary">Edit</a>
